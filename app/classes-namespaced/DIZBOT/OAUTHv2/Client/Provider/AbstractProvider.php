...
use DIZBOT\OAUTHv2\Client\Grant\AbstractGrant;
use DIZBOT\OAUTHv2\Client\Grant\GrantFactory;
use DIZBOT\OAUTHv2\Client\OptionProvider\OptionProviderInterface;
use DIZBOT\OAUTHv2\Client\OptionProvider\PostAuthOptionProvider;
...
use DIZBOT\OAUTHv2\Client\Tool\ArrayAccessorTrait;
use DIZBOT\OAUTHv2\Client\Tool\GuardedPropertyTrait;
use DIZBOT\OAUTHv2\Client\Tool\QueryBuilderTrait;
use DIZBOT\OAUTHv2\Client\Tool\RequestFactory;
...
abstract class AbstractProvider
{
    ...
    public function getCurrentAccessTokenFromStorage() {

        $this->storage->query("SELECT * from api where tag='" . $this->call_tag . "' ORDER BY ID DESC LIMIT 1");
         
        $results = $this->storage->fetchAll();
        if(count($results) > 0) {
           
            return unserialize($results[0]['accessTokenObject']);
        }
        return false;
    }
    public function receiveAccessTokenToStorage($accessToken) {
        $accessTokenVal = $accessToken->getToken();
        $refreshToken = $accessToken->getRefreshToken();
        $expires = $accessToken->getExpires();
        $this->storage->query("UPDATE api SET access_token=?,refresh_token=?,accessTokenObject=?,x_refresh_token_expires=? WHERE tag=?",$accessTokenVal,$refreshToken,serialize($accessToken),$expires,$this->call_tag);
        if($this->storage->affectedRows() === 0) {
            $this->storage->query("INSERT INTO api (tag,access_token,refresh_token,accessTokenObject,x_refresh_token_expires) VALUES (?,?,?,?,?)",$this->call_tag,$accessTokenVal,$refreshToken,serialize($accessToken),$expires);
        }
    }

    public function getOrRefreshAccessToken() {

        $currentToken = $this->getCurrentAccessTokenFromStorage();
       
        if($currentToken !== false && $currentToken->hasExpired()) {
            $currentToken = $this->getAccessToken('refresh_token', [ 'refresh_token' => $currentToken->getRefreshToken() ]);
            $this->receiveAccessTokenToStorage($currentToken);
        }
        
        return $currentToken;
    }

    public function performInitialAuthorization($code = null,$state = null) {
        ...
        elseif (empty($state) || (isset($_SESSION['oauth2state']) && $state !== $_SESSION['oauth2state'])) {
            ...
            try {
                ...
                $this->receiveAccessTokenToStorage($accessToken);
                ...
                //     $accessToken
                // );
            } catch (\DIZBOT\OAUTHv2\Client\Provider\Exception\IdentityProviderException $e) {
                exit($e->getMessage());
            }
    ...
    public function getResponse(RequestInterface $request)
    {
        ...
        $response = $this->getHttpClient()->sendRequest($request);  // DEBUG - This used to call ->send($request)
